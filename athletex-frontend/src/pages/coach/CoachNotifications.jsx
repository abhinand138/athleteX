...
export default function CoachNotifications() {
  ...
  const handleSendAnnouncement = async (e) => {
    ...
    try {
      const payload = {
        coachId,
        sendToAllRoster: sendToAll,
        recipientIds: sendToAll ? [] : [selectedAthleteId],
        title: announcementTitle.trim(),
        message: announcementMessage.trim()
      };

      await api.post("/notifications/announcement", payload);
      ...
      toast.error(err.response?.data || "Failed to broadcast announcement.");
    } finally {
      setSending(false);
    }
  };
      ...
      <div className="max-w-7xl mx-auto space-y-8 pb-16 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div className="relative">
            ...
            <p className="text-gray-400 text-base font-medium ml-5">
              Stay updated with your athletes and broadcast roster announcements
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 shrink-0 flex-wrap">
            <button
              ...
              className="flex items-center gap-2 px-6 py-3 rounded-2xl bg-gradient-to-r from-brand-peach to-orange-500 hover:from-brand-peach/90 hover:to-orange-500/90 text-black font-bold text-xs uppercase tracking-wider transition-all shadow-[0_0_20px_rgba(255,123,84,0.3)] cursor-pointer"
            >
              <FaBullhorn />
              + New Announcement
            </button>
        ...
        {loading ? (
          <div className="flex justify-center items-center h-64">
            <div className="flex flex-col items-center gap-3">
              ...
              <p className="text-gray-400 text-xs font-medium animate-pulse">Loading notification feed...</p>
            </div>
          </div>
        ) : error ? (
          ...
          <div className="glass-card rounded-3xl p-12 border border-white/5 text-center max-w-lg mx-auto shadow-2xl">
            ...
            <p className="text-gray-400 text-xs leading-relaxed">
              {activeFilter === "UNREAD"
                ...
                : "You're all caught up. Updates on workouts, achievements, and announcements will appear here."}
            </p>
          </div>
        ) : (
        ...
        {isModalOpen && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
            <div className="glass-card rounded-3xl p-8 border border-white/10 max-w-lg w-full shadow-2xl relative animate-fadeIn">
              <div className="flex items-center justify-between mb-6 pb-4 border-b border-white/10">
                <div className="flex items-center gap-3">
                  <div className="p-3 bg-brand-peach/10 text-brand-peach rounded-2xl text-xl">
                    <FaBullhorn />
                  </div>
                  <div>
                    <h3 className="text-xl font-bold text-white">Broadcast Announcement</h3>
                    <p className="text-xs text-gray-400">Send an instant alert to your assigned athletes</p>
                  </div>
                </div>
                <button
              ...
              {announcementSuccess && (
                <div className="mb-4 p-3.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-semibold text-center">
                  {announcementSuccess}
                </div>
              )}
              ...
              <form onSubmit={handleSendAnnouncement} className="space-y-5">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <button
                      ...
                      className={`p-3 rounded-xl border text-xs font-bold flex items-center justify-center gap-2 transition-all cursor-pointer ${
                        sendToAll
                          ? "bg-brand-peach text-black border-brand-peach shadow-md"
                          : "bg-white/5 text-gray-400 border-white/10 hover:text-white"
                      }`}
                    >
                      <FaUsers />
                      Entire Active Roster ({assignedAthletes.length})
                    </button>
                ...
                {!sendToAll && (
                  <div>
                    ...
                    <select
                      ...
                      className="w-full bg-[#111317] border border-white/10 rounded-xl px-4 py-2.5 text-white text-xs font-semibold focus:outline-none focus:border-brand-peach/50"
                    >
                ...
                <div>
                  ...
                  <input
                    type="text"
                    required
                    placeholder="e.g. Schedule Update: Evening Training Postponed"
                    value={announcementTitle}
                    onChange={(e) => setAnnouncementTitle(e.target.value)}
                    className="w-full bg-[#111317] border border-white/10 rounded-xl px-4 py-2.5 text-white text-xs placeholder:text-gray-600 focus:outline-none focus:border-brand-peach/50"
                  />
                </div>
                ...
                <div>
                  <div className="flex justify-between items-center mb-1.5">
                    ...
                    <span className="text-[10px] text-gray-500">{announcementMessage.length}/500</span>
                  </div>
                  <textarea
                    required
                    rows={4}
                    maxLength={500}
                    placeholder="Provide details about the session change, team briefing, or important announcement..."
                    value={announcementMessage}
                    onChange={(e) => setAnnouncementMessage(e.target.value)}
                    className="w-full bg-[#111317] border border-white/10 rounded-xl p-3.5 text-white text-xs placeholder:text-gray-600 focus:outline-none focus:border-brand-peach/50 resize-none"
                  />
                </div>
                ...
                <div className="flex items-center justify-end gap-3 pt-4 border-t border-white/10">
                  ...
                  <button
                    type="submit"
                    disabled={sending}
                    className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-brand-peach hover:bg-brand-peach/90 text-black text-xs font-bold uppercase tracking-wider transition-all shadow-md cursor-pointer disabled:opacity-50"
                  >
                    <FaPaperPlane />
